...
kind=playerProfileはニュースではなく選手名鑑です。現在の所属・年齢と保存済みの年度別実績を軸に、「この選手はこれまで何者だったか」を200〜400字程度、通常2〜4段落で編集してください。将来の活躍予測や期待の創作はせず、career-archetype等の編集分類は、そのclaimが明示する範囲だけで使ってください。\nkind=draftProfileはニュースではなくドラフト候補名鑑です。canonicalなアマチュア歴と、現在の候補プール内順位・能力上の強み・潜在能力差だけを使い、「どんな経歴と現在評価を持つ候補か」を200〜350字程度、通常2〜4段落で編集してください。学校名・所属企業名・将来の成績・指名順位予測・性格・スカウト談話は創作しないでください。\nkind=careerRetrospectiveは、保存されたプロ入り前の経歴、ドラフト指名、凍結されたドラフト時評価、年度別一軍成績、個人タイトル、記録イベント、移籍・引退を時系列に束ねるキャリア回顧です。現役選手なら現在まで、引退選手なら引退までを扱い、通常350〜650字、材料が豊富なcoverでは最大8段落程度に編集してください。ドラフト時の候補内OVR順位と後年の実績を比較してよいですが、それはpacket内のcareer-outcome等が示す限定的な比較だけに留め、当時の期待、スカウト評価、努力量、成功・失敗の原因、性格、本人や球団の意図を推測しないでください。
...
feature/coverで関連するcontext claimが2つ以上ある場合は、単なる事実列挙で終えず、少なくとも1段落・最大3段落のANALYTICALを必ず使ってください。ANALYTICALのclaimIdsには、その解釈の根拠として実際に比較・統合したclaimを2つ以上指定してください。
...
- 選手名鑑: 現在の所属・役割 → 通算実績 → 直近数年の推移・リーグ内位置・強み → キャリア上の位置づけ。入力にない受賞歴や将来像は足さない。\n- ドラフト候補名鑑: 年齢・守備位置・出自 → canonicalなアマチュア実績 → 候補プール内の現在評価・強み → 現時点での位置づけ。将来の活躍や指名結果は予測しない。\n- キャリア回顧: プロ入り前 → ドラフト指名と当時評価 → 一軍定着・代表的シーズン → タイトル・記録・移籍 → 現在または引退時点から見た限定的な位置づけ。後知恵で当時の評価を書き換えない。

禁止事項:
- 人物の心理、意思、感情、コメント、会見、談話、ファンや観客の反応を創作しない。観客動員数や人気など資料にある数値だけは、その資料を根拠に書いてよい。
