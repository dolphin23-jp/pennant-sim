...
function extractLegacyLogic(html) {
  ...
  if (!scriptMatch) throw new Error('Could not find the Babel script in legacy/index.html.');
  const uiBoundary = scriptMatch[1].indexOf('UI PARTS');
  if (uiBoundary < 0) throw new Error('Could not find the UI PARTS boundary in legacy/index.html.');
  const commentStart = scriptMatch[1].lastIndexOf('/*', uiBoundary);
  if (commentStart < 0) throw new Error('Could not isolate the legacy non-UI logic.');
...
;globalThis.__legacyEngine = {
  ...
  new vm.Script(`${legacyLogic}\n${exportSource}`, { filename: 'legacy/index.html' }).runInContext(
    context,
  );
...
function finalizeSeason(accumulatedStats, games) {
  ...
  const atBats = total(batting, 'ab');
  const hits = total(batting, 'h');
  const homeRuns = total(batting, 'hr');
  const stolenBases = total(batting, 'sb');
  const caughtStealing = total(batting, 'cs');
  const walks = total(batting, 'bb');
  const plateAppearances = total(batting, 'pa');
  const earnedRuns = total(pitching, 'er');
  const pitchingOuts = total(pitching, 'ip3');
  return {
    games,
...
const roundSummary = (summary, digits) =>
  Object.fromEntries(
...
async function main() {
  ...
  console.log(`Legacy NPB target evaluation: ${JSON.stringify(output.targetEvaluation)}`);
