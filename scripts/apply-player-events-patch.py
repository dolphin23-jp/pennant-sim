...
def patch(path: str, old: str, new: str) -> None:
    ...
    if new in text:
        ...
        return
    if old not in text:
        print(f'pattern missing: {path}: {old[:80]!r}')
        return
    target.write_text(text.replace(old, new, 1))
    print(f'patched: {path}')


constants = Path('src/data/constants.ts')
text = constants.read_text()
if 'PLAYER_DEVELOPMENT_BALANCE' not in text:
    marker = "export const MATURITY_WEIGHTS = [0.08, 0.18, 0.44, 0.18, 0.12];\n"
    addition = marker + """
...
"""
    ...
    constants.write_text(text.replace(marker, addition, 1))
...
Path('src/engine/playerEvents.ts').write_text("""import { PLAYER_DEVELOPMENT_BALANCE } from '../data';
...
patch('src/engine/index.ts', "export * from './market';", "export * from './market';\nexport * from './playerEvents';")
...
required = {
    'src/data/constants.ts': ['PLAYER_DEVELOPMENT_BALANCE'],
    'src/engine/growth.ts': ['developmentGrowth', 'eligiblePlayerIds'],
    'src/engine/playerEvents.ts': ['applyPostGamePlayerEvents'],
    'src/engine/types.ts': ['postGameEvents: PostGameEvents'],
    'src/engine/game.ts': ['applyPostGamePlayerEvents', 'participantIds'],
    'src/engine/index.ts': ["export * from './playerEvents'"],
}
...
print('implementation patch complete')
