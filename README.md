...
現行ゲームにはCPU球団の自動引退処理がないため、この診断だけは全球団へ簡易ルールを適用します。40歳以上、または35歳以上かつOVR 50以下の選手を対象にし、さらに毎年6巡のドラフト枠を空けるため低評価選手を整理します。ドラフト後に投手・野手の人数が初期ロースター上限を超えた場合も、低評価順に上限まで整理します。このルールは `scripts/franchise-drift.ts` 内だけにあり、プレイヤー向けオフシーズンUIや実際のゲーム進行は変更しません。

各年について開幕時とドラフト後の野手・投手平均OVR、球団間OVR格差、平均年齢、引退・指名人数に加え、リーグ打率、BABIP、防御率、本塁打、得点、四球率、三振率を記録します。
