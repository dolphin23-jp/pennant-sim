...
function gaussian(random: () => number, mean = 0, standardDeviation = 1): number {
  ...
  const second = random();
  return (
    mean + Math.sqrt(-2 * Math.log(first)) * Math.cos(2 * Math.PI * second) * standardDeviation
  );
}
...
function generateBatterStats(player: Player, age: number, random: () => number): BatterStats {
  ...
  const average = clamp(
    0.205 + contact * 0.00105 * factor + gaussian(random, 0, 0.014),
    0.16,
    0.385,
  );
  const hits = Math.min(ab, Math.round(ab * average));
  const homeRuns = clamp(
    Math.round((((power - 35) * games) / 265) * factor + gaussian(random, 0, 3.2)),
    0,
    68,
  );
  const doubles = clamp(
    Math.round(hits * (0.16 + power / 1000) + gaussian(random, 0, 3)),
    0,
    hits - homeRuns,
  );
  ...
  const walks = clamp(
    Math.round(pa * (0.045 + discipline * 0.00075) + gaussian(random, 0, 6)),
    0,
    pa - ab,
  );
  const strikeouts = clamp(
    Math.round(pa * (0.25 - contact * 0.00125) + gaussian(random, 0, 9)),
    0,
    pa,
  );
  return {
...
function generatePitcherStats(player: Player, age: number, random: () => number): PitcherStats {
  ...
  const starts =
    role === '先発' ? clamp(Math.round(22 * factor + gaussian(random, 0, 5)), 3, 31) : 0;
  const games =
    role === '先発'
      ? starts
      : clamp(
          Math.round((role === 'クローザー' ? 53 : 47) * factor + gaussian(random, 0, 9)),
          8,
          72,
        );
  const innings =
  ...
  const hits = clamp(
    Math.round(innings * clamp(1.12 - ability * 0.0045 + gaussian(random, 0, 0.05), 0.55, 1.2)),
    0,
    260,
  );
  const walks = clamp(
    Math.round(
      innings *
        clamp(0.58 - (player.p.ctrl ?? 50) * 0.006 + gaussian(random, 0, 0.035), 0.08, 0.65),
    ),
    0,
    130,
  );
  const strikeouts = clamp(
    Math.round(
      innings *
        clamp(
          0.45 + ((player.p.vel ?? 50) + (player.p.nobi ?? 50)) * 0.005 + gaussian(random, 0, 0.08),
          0.35,
          1.45,
        ),
    ),
  ...
  const wins =
    role === '先発'
      ? clamp(Math.round(starts * 0.42 + gaussian(random, 0, 2.1)), 0, 24)
      : clamp(Math.round(games / 22 + gaussian(random, 0, 1.3)), 0, 10);
  return {
    ...
    l:
      role === '先発'
        ? clamp(Math.round(starts * 0.34 + gaussian(random, 0, 2)), 0, 20)
        : clamp(Math.round(games / 28 + gaussian(random, 0, 1)), 0, 9),
    ...
    bs:
      role === 'クローザー' ? clamp(Math.round(games * 0.08 + gaussian(random, 0, 1.5)), 0, 12) : 0,
    ip3,
    h: hits,
    bb: walks,
    k: strikeouts,
...
function generateStats(player: Player, age: number, random: () => number): PlayerStats {
  return player.isP
    ? generatePitcherStats(player, age, random)
    : generateBatterStats(player, age, random);
}
...
function createLegend(
  ...
  const surname =
    LEGEND_SURNAMES[
      (index * 5 + randomInt(random, 0, LEGEND_SURNAMES.length - 1)) % LEGEND_SURNAMES.length
    ];
  const given =
    LEGEND_GIVEN_NAMES[
      (index * 7 + randomInt(random, 0, LEGEND_GIVEN_NAMES.length - 1)) % LEGEND_GIVEN_NAMES.length
    ];
...
export function createFictionalLeagueHistory(
  ...
  for (const [teamKey, team] of entries) {
    for (const player of [...team.fielders, ...team.pitchers]) {
      ...
      const proYears =
        possibleYears === 0 ? 0 : clamp(possibleYears - randomInt(random, 0, 3), 1, possibleYears);
    ...
    const templates = [...team.fielders, ...team.pitchers].sort(
      (first, second) => calcOVR(second) - calcOVR(first),
    );
  ...
  for (let year = startYear; year <= endYear; year += 1) {
    ...
    championHistory.push({
      ...
      champion,
      runnerUp,
      keyBatters: championTeam.fielders
        .slice()
        .sort((a, b) => calcOVR(b) - calcOVR(a))
        .slice(0, 2)
        .map((player) => player.name),
      keyPitchers: championTeam.pitchers
        .slice()
        .sort((a, b) => calcOVR(b) - calcOVR(a))
        .slice(0, 2)
        .map((player) => player.name),
